...
Z00; исходная строка или её base64-копия не дублируется.

При импорте запись с `DEL$a=Y` целиком исключается до валидации;
количество пропусков — `skippedDeletedRecords` в статистике CLI/API.
Буквенные поля исключаются из `fields`; `LDR` и `FMT` переходят в
`leader` и `format`. Leader/19 допускает `a`, `b`, `c`, проверка Leader/18
сохранена. Подробнее: [политика импорта](docs/RSL-import-policy.md).

В обоих направлениях ошибки валидации не заменяют исходные значения и не
останавливают обработку. Неизвестный FMT, недопустимые коды Leader, тегов,
индикаторов и подполей сохраняются; нарушения записываются в отчёт.
После успешного завершения рядом с результатом создаётся файл
`<имя результата>.validation-errors.ndjson`, если есть ошибки. Каждая строка
содержит `recordIndex` (с нуля), `byteOffset` и массив `errors`.
При отсутствии FMT значение `format` — пустая строка.
Структурные ошибки (длины, границы полей, Directory, разделители) прерывают
конвертацию; частичный результат не публикуется, предыдущий результат сохраняется.
В API ссылка `validationErrorsUrl` ведёт на
`GET /jobs/:jobId/validation-errors`; worker загружает отчёт в хранилище.
...
[Миграция логики валидации полей](docs/Миграция%20логики%20валидации%20полей.md):
разделение структурных исключений и ошибок профиля, изменения регулярок,
карта проверок со ссылками на строки и ограничения текущей реализации.

[Та же статья в wiki](https://github.com/mbob72/marc_parser/wiki/Validation-migration).
