...
lines += ['## Интерпретация для продуктовой команды', '',
          ...
          '2. **Исторический профиль LD-02.** Код прогона запрещал Leader/18 `c`, `n` и '
          ...
          'В этом историческом прогоне VF-G5 и SF-G4 отмечали один и тот же недопустимый код подполя, '
          ...
          '## Публикация', '',
          'Страница GitHub Wiki: `RSL-validation-2026-09-17`. Основной репозиторий хранит реестры, скрипты и LFS-указатели; Wiki публикуется отдельным коммитом в её репозиторий. Перед удалением локальных исходников проверяются удалённый коммит и доступность LFS-объектов с ожидаемыми SHA-256 и размерами.', '']
pathlib.Path('docs/RSL-validation-2026-09-17.md').write_text(annotate_rule_status('\n'.join(lines)))
